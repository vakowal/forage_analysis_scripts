...
for site in site_list:
    ...
    cent.write_century_bat(century_dir, century_bat, schedule, output,
                           fix_file, outvars)
    ...
    try:
        with open(summary_csv, 'wb') as summary_file:
            ...
            for iter in xrange(max_iterations):
                ...
                if output[-4:] == '.lis':
                    output_log = os.path.join(century_dir, output[:-4] +
                                              '_log.txt')
                else:
                ...
                if float(abs(simulated_biomass - empirical_biomass)) <= \
                    ...
                    if vary == 'intensity':
                        ...
                        if target_dict:
                            ...
                            cent.modify_schedule(schedule_file,
                                                 increase_intensity,
                                                 target_dict, 'GL', out_dir,
                                                 site['name'] + str(iter))
                        else:
        ...
        shutil.copyfile(os.path.join(century_dir, 'sch_orig.100'),
                        schedule_file)
