...
def write_site_files_mongolia(template, soil_table, save_dir):
    ...
    for inputs_dict in in_list:
            ...
            with open(template, 'rb') as old_file:
                ...
                for line in old_file:
                    if '  \'SITLAT' in line:
                        ...
                        num = inputs_dict['sand_0_20_cm'] / 100.0
                        item = '{:0.12f}'.format(num)[:7]
                        newline = '%s           \'SAND\'\r\n' % item
                    elif '  \'SILT' in line:
                        num = inputs_dict['silt_0_20_cm'] / 100.0
                        item = '{:0.12f}'.format(num)[:7]
                        newline = '%s           \'SILT\'\r\n' % item
                    elif '  \'CLAY' in line:
                        num = inputs_dict['clay_0_20_cm'] / 100.0
                        item = '{:0.12f}'.format(num)[:7]
                        newline = '%s           \'CLAY\'\r\n' % item
                    elif '  \'BULKD' in line:
        ...
        save_as = os.path.join(save_dir, '{}.100'.format((inputs_dict['site'])))
...
def make_sch_files_mongolia():
    ...
    def copy_sch_file(template, site_name, save_as):
        fh, abs_path = mkstemp()
        os.close(fh)
        with open(abs_path, 'wb') as newfile:
            with open(template, 'rb') as old_file:
